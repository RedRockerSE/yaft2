...
The Core API provides methods to extract metadata from iOS Keychain and Android Keystore/locksettings databases. These methods currently focus on **metadata extraction only** - they do not decrypt encrypted credentials.
...
YAFT includes a flexible plugin update system that supports both online (GitHub) and local/network folder sources. This allows users to easily keep plugins up-to-date, download new plugins, or use plugins from a local/network folder for air-gapped environments.

### Configuration

The plugin update system is configured via `config/plugin_updater.toml`. This configuration file allows you to choose between online and local sources, and customize update behavior.

**Configuration File** (`config/plugin_updater.toml`):

```toml
[plugin_updater]
# Source type: online, local
# online: Download plugins from GitHub repository (default)
# local: Load plugins from local or network folder
source_type = "online"

# Check interval in hours (how often to check for updates)
check_interval_hours = 24

# Request timeout in seconds (for online sources)
timeout = 30

# Verify plugin integrity using SHA256 hashes
verify_integrity = true

# Backup existing plugins before overwriting
backup_on_update = true

# Online source configuration (used when source_type = "online")
...
# Path to local or network folder containing plugins
# Examples:
#   Windows local: "C:\\Users\\Username\\plugins"
#   Windows network: "\\\\SERVER\\share\\plugins"
#   Linux/macOS: "/home/username/plugins" or "/mnt/network/plugins"
#
# The folder should contain:
#   - Plugin Python files (*.py)
#   - plugins_manifest.json (manifest file with plugin metadata)
path = ""

# Whether to automatically create manifest for local plugins if missing
auto_generate_manifest = false
```

**Using Local/Network Sources:**

To use a local or network folder as your plugin source:

1. Set `source_type = "local"` in `config/plugin_updater.toml`
2. Set the `path` to your local or network folder (Windows: use `\\` for backslashes)
3. Place plugin files (*.py) in the folder
4. Optionally place `plugins_manifest.json` in the folder (or enable `auto_generate_manifest`)
...
[plugin_updater.local]
path = "C:\\Users\\Forensic\\my_plugins"
...
[plugin_updater.local]
path = "\\\\FORENSIC-SERVER\\shared\\yaft_plugins"
auto_generate_manifest = false
```

### Architecture: Hybrid Approach with Manifest

The plugin update system uses a manifest-based approach that balances performance, security, and offline capability. It supports both online (GitHub) and local/network sources:

**Online Sources (GitHub):**
...
1. **Manifest File** (`plugins_manifest.json`): Loaded from local folder or auto-generated
2. **Direct File Access**: Copies plugins directly from local/network path
3. **SHA256 Verification**: Ensures integrity of copied files
4. **Air-Gapped Support**: Works in environments without internet access
5. **Network Share Support**: Compatible with Windows UNC paths (\\\\server\\share)
...
Plugins can programmatically access the update system. The system automatically uses configuration from `config/plugin_updater.toml`:

```python
# Get PluginUpdater instance (uses config from config/plugin_updater.toml)
updater = self.core_api.get_plugin_updater()

# Check for updates (works with both online and local sources)
...
if result.updates_available:
    ...
    # Download updates (downloads from GitHub or copies from local folder)
...
# Use custom configuration (override config file)
from yaft.core.plugin_updater import PluginUpdaterConfig

custom_config = PluginUpdaterConfig(source_type="local")
custom_config.local.path = "C:\\path\\to\\custom\\plugins"
custom_config.local.auto_generate_manifest = True
